...
class HyperspaceTunnelBackground implements ThreeThemeRuntime {
  ...
  private updateAnimationTimeline() {
  ...
  private updateCurve() {
    ...
    for (let i = 0; i < tubePositions.count; i += 1) {
      ...
      tubePositions.setXYZ(
        i,
        cx + (ox + sx - cx) / 15, // smoothly interpolate current X position toward (Original_X + Spline_Target_X)
  ...
  private easePower1InOut(t: number) {
    const x = this.clamp(t, 0, 1);
    return x < 0.5 ? 2 * x * x : 1 - Math.pow(-2 * x + 2, 2) / 2;
  }

  private easePower2InOut(t: number) {
    const x = this.clamp(t, 0, 1);
    return x < 0.5 ? 4 * x * x * x : 1 - Math.pow(-2 * x + 2, 3) / 2;
  }

  private roughEase(t: number, seed: number) {
    const x = this.clamp(t, 0, 1);
    const jitter = (
      Math.sin((x * 37.0 + seed * 11.13) * Math.PI) * 0.09 +
      Math.sin((x * 91.0 + seed * 3.71) * Math.PI) * 0.04
    ) * Math.sin(Math.PI * x);
