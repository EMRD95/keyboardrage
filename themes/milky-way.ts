...
function normalizeLetters(value: string, stripAccents = false) {
  let normalized = value.trim().toLowerCase().normalize(stripAccents ? 'NFKD' : 'NFKC');
  if (stripAccents) normalized = normalized.replace(/[\u0300-\u036f]/g, '');
...
function getStarMetrics(pointCount: number) {
...
export class MilkyWayBackground implements ThreeThemeRuntime {
  private readonly mainScene: THREE.Scene;
  private readonly renderer: THREE.WebGLRenderer;
  private readonly spaceScene = new THREE.Scene();
  private readonly spaceCamera: THREE.PerspectiveCamera;
  private readonly renderTarget: THREE.WebGLRenderTarget;
  private readonly display: THREE.Mesh<THREE.PlaneGeometry, THREE.ShaderMaterial>;
  private readonly displayUniforms: { uTexture: { value: THREE.Texture }; uOpacity: { value: number } };

  private readonly galaxyRoot = new THREE.Group();
  ...
  private readonly starTexture = makeRadialTexture(128);
  private readonly haloTexture = makeRadialTexture(192, 'rgba(255,255,255,0.95)');
  private readonly wideGlowTexture = makeWideGlowTexture(256);
  ...
  constructor(context: ThreeThemeContext, renderOrder = MILKY_WAY_RENDER_ORDER) {
  ...
  setVisible(visible: boolean) {
    ...
    if (visible) this.renderSpaceToTarget();
  }
  ...
  ready(info: ThreeThemeReadyInfo = {}): Promise<void> {
    ...
    this.loadPromise = loadGalaxyData(language)
      .then((data) => {
        ...
        this.buildPointCloudGeometry();
        this.hideActiveStar();
        if (this.visible) this.renderSpaceToTarget();
      })
      .catch((error) => {
  ...
  private buildRawPreprocessedPositions() {
  ...
  private rebuildPointColors() {
    ...
    if (this.activeIndex >= 0) this.paintActivePoint(this.activeIndex);
  ...
  private hideActiveStar() {
    this.activeGroup.visible = false;
  }

  private addWordIndex(word: string, index: number) {
    lookupKeys(word).forEach((key) => {
      if (key && !this.wordIndex.has(key)) this.wordIndex.set(key, index);
    });
  }
  ...
  private moveActiveStarToPoint(index: number) {
...
export const milkyWayTheme: CustomThreeThemeDefinition = {
  id: 'milky-way',
  label: 'Galaxy (3D)',
  kind: 'three',
