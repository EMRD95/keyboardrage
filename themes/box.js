...
class CastleTunnelBackground {
    constructor(context, renderOrder) {
        ...
        // Changed repeatY to 4 so that each of the 4 flat walls maps cleanly to the texture
        this.textureParams = { offsetX: 0, offsetY: 0, repeatX: 30, repeatY: 4 };
        ...
        this.tunnelRenderTarget = new THREE.WebGLRenderTarget(Math.floor(context.logicalWidth * pixelRatio), Math.floor(context.logicalHeight * pixelRatio), { depthBuffer: true, stencilBuffer: false });
    ...
    loadCastleTextures() {
        ...
        const tryCreate = () => {
            ...
            if (loadedCount < 2)
                return;
            if (!stoneTexture || !bumpTexture)
                return;
            this.createTunnelMesh(stoneTexture, bumpTexture);
        };
        loader.load('/textures/stonePattern.jpg', (texture) => {
            ...
            tryCreate();
        }, undefined, (error) => console.error('Castle: stone texture load error', error));
        loader.load('/textures/stonePatternBump.jpg', (texture) => {
            texture.wrapS = THREE.RepeatWrapping;
            texture.wrapT = THREE.RepeatWrapping;
            texture.repeat.set(this.textureParams.repeatX, this.textureParams.repeatY);
            texture.needsUpdate = true;
            bumpTexture = texture;
            tryCreate();
        }, undefined, (error) => console.error('Castle: bump texture load error', error));
    }
    createTunnelMesh(stoneTexture, bumpTexture) {
        if (this.tunnelMesh) {
            this.tunnelScene.remove(this.tunnelMesh);
            this.tunnelTubeGeometry?.dispose();
            this.tunnelSplineMesh?.geometry.dispose();
        ...
        // CRITICAL FIX: Changing radial segments to 4 to create a box instead of a tube.
        this.tunnelTubeGeometry = new THREE.TubeGeometry(this.tunnelCurve, 70, 0.04, 4, false);
        // Rotate the geometry itself 45 degrees so that the square corners form a proper room
        // (Floor, Ceiling, Left Wall, Right Wall) instead of a diamond shape.
    ...
    updateCurve() {
        ...
        // CRITICAL FIX: Since we lowered radial segments to 4, our new stride is 5
        // (4 segments + 1 duplicate seam vertex for UV mapping)
        const ringStride = 5;
