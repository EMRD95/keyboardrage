...
export type GalaxyWordPoint = readonly [word: string, x: number, y: number, z: number];

export type GalaxyPointSet = {
  points: readonly GalaxyWordPoint[];
  sourceIndexToPointIndex: ReadonlyMap<number, number>;
  totalWords: number;
  visibleDots: number;
  /** Changing this does NOT rebuild geometry — it only gates active-word routing. */
  frequencyLimit: number;
  /** Precomputed position buffer — use directly instead of buildExpandedBoxPointPositions. */
  positions: Float32Array;
  /** Precomputed base color buffer — use directly instead of rebuildPointColors. */
  baseColors: Float32Array;
  /** Axis stats matching buildExpandedBoxPointPositions — center + halfRange per axis. */
  axisStats: readonly [AxisStats, AxisStats, AxisStats];
};
...
function precomputeBaseColors(points: readonly GalaxyWordPoint[], count: number): Float32Array {
...
function computeAxisStats(points: readonly GalaxyWordPoint[], count: number, axis: 1 | 2 | 3): AxisStats {
  const values = new Float64Array(count);
  for (let i = 0; i < count; i += 1) values[i] = points[i][axis];
  values.sort();
...
function expandPosition(value: number, stats: AxisStats, targetHalfExtent: number): number {
...
function precomputePositions(
  points: readonly GalaxyWordPoint[], count: number,
  stats: readonly [AxisStats, AxisStats, AxisStats], targetHalfExtent: number
): Float32Array {
...
class GalaxyData {
  ...
  private fullPointSet: GalaxyPointSet | null = null;
  ...
  getPointSet(frequencyLimit: number | undefined): GalaxyPointSet {
    ...
    const axisStats: [AxisStats, AxisStats, AxisStats] = [
  ...
  pointForSourceIndex(sourceIndex: number): GalaxyWordPoint {
...
const dataCache = new Map<string, Promise<GalaxyData>>();
...
async function loadGalaxyDataNow(language: string): Promise<GalaxyData> {
  const wordsUrl = `/words/${language}/words.json`;
  const wordsPayload = await fetchJson<WordsPayload>(wordsUrl);
  ...
  if (magic !== 'KRG1') throw new Error(`Unsupported galaxy coordinate format: ${magic}`);
  ...
  for (let i = 0; i < raw.length; i += 1) coords[i] = (raw[i] / 32767) * COORDINATE_SCALE;
  return new GalaxyData(wordsPayload.words, coords);
}

export function loadGalaxyData(language: string): Promise<GalaxyData> {
...
export function isGalaxyLanguage(language: string | undefined): boolean {
