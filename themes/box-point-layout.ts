export type BoxPointTuple = readonly [word: string, x: number, y: number, z: number];
...
function axisStats(values: number[], targetHalfExtent: number): AxisStats {
...
export function buildExpandedBoxPointPositions(points: readonly BoxPointTuple[], targetHalfExtent: number) {
  ...
  const stats = [axisStats(xs, target), axisStats(ys, target), axisStats(zs, target)] as const;
  const expand = (value: number, axis: AxisStats) => {
