...
export function textDirectionForLanguage(language) {
 ...
 */
export function needsShapedRendering(text) {
    return SHAPED_SCRIPT_RE.test(text);
}
export function usesScriptAwareSpeed(language, words = []) {
    const normalized = normalizeLanguage(language);
    if (CJK_LANGUAGES.has(normalized) || RTL_LANGUAGES.has(normalized))
        return true;
...
export function estimateTypingEffort(text) {
    let effort = 0;
    for (const char of text.trim()) {
        ...
        if (HAN_RE.test(char))
            effort += 2.5;
        else if (HANGUL_RE.test(char) || HIRAGANA_RE.test(char) || KATAKANA_RE.test(char))
            effort += 2.0;
        else if (THAI_LIKE_RE.test(char) || INDIC_RE.test(char))
            effort += 1.35;
        else
            effort += 1;
    }
...
export function averageTypingEffort(words, fallbackAverage) {
    if (words.length === 0)
        return fallbackAverage;
    const total = words.reduce((sum, word) => sum + estimateTypingEffort(word), 0);
    return total / words.length;
}
export function effectiveAverageWordLength(language, words, fallbackAverage) {
    if (!usesScriptAwareSpeed(language, words))
        return fallbackAverage;
    const normalized = normalizeLanguage(language);
